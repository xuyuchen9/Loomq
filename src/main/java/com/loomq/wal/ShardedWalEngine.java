...
public class ShardedWalEngine implements AutoCloseable {
    ...
    // 分片写入器映射：shardId -> WalWriter
    private final Map<String, WalWriter> shardWriters;
    ...
    public void stop() {
        ...
        // 停止所有分片写入器
        for (Map.Entry<String, WalWriter> entry : shardWriters.entrySet()) {
            try {
                entry.getValue().stop();
     ...
     * @return WalWriter
     */
    private WalWriter getOrCreateWriter(String shardId) throws IOException {
        return shardWriters.computeIfAbsent(shardId, id -> {
            try {
                WalWriter writer = new WalWriter(config, id);
                writer.start();
    ...
    public CompletableFuture<Long> appendAsync(String shardId, String taskId, String bizKey,
        ...
        try {
            WalWriter writer = getOrCreateWriter(shardId);
     ...
     * @return 统计信息
     */
    public WalWriter.Stats getStats(String shardId) {
        WalWriter writer = shardWriters.get(shardId);
    ...
    public void close() throws IOException {
        ...
        // 关闭所有写入器
        for (WalWriter writer : shardWriters.values()) {
            try {
                writer.close();
            } catch (IOException e) {
